System.register(['@angular/core', './emp.service', './shared.service'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var core_1, emp_service_1, shared_service_1;
    var NewListComponent;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (emp_service_1_1) {
                emp_service_1 = emp_service_1_1;
            },
            function (shared_service_1_1) {
                shared_service_1 = shared_service_1_1;
            }],
        execute: function() {
            NewListComponent = (function () {
                function NewListComponent(empLsSrv, sharedSrv) {
                    this.empLsSrv = empLsSrv;
                    this.sharedSrv = sharedSrv;
                }
                NewListComponent.prototype.ngOnInit = function () {
                    this.newLst = this.empLsSrv.newList;
                };
                NewListComponent.prototype.showNewList = function () {
                    this.empLsSrv.setTheRecentList(this.newLst);
                };
                NewListComponent.prototype.updateAllMsg = function () {
                    this.sharedSrv.changeMessage("new Message from Shared Service");
                };
                NewListComponent = __decorate([
                    core_1.Component({
                        selector: 'new-list-comp',
                        templateUrl: './partials/new-list-comp.html'
                    }), 
                    __metadata('design:paramtypes', [emp_service_1.EmpService, shared_service_1.SharedService])
                ], NewListComponent);
                return NewListComponent;
            }());
            exports_1("NewListComponent", NewListComponent);
        }
    }
});
//# sourceMappingURL=new.list.comp.js.map